/**
 * Example script: Check token balance for an address.
 *
 * Usage:
 *   npx tsx check-balance.ts
 *
 * Requires .env with deployed contract details.
 */

import * as path from "path";
import { config } from "dotenv";
import { hd, helpers, config as lumosConfig } from "@ckb-lumos/lumos";

config({ path: path.resolve(__dirname, "../.env") });

import { CkbClient } from "../mcp-sdk/src/client";
import { getTokenBalance } from "../mcp-sdk/src/tools/balance";
import { computeScriptHash } from "../mcp-sdk/src/utils/hash";

const CKB_RPC_URL = process.env.CKB_RPC_URL || "http://127.0.0.1:8114";
const CKB_INDEXER_URL = process.env.CKB_INDEXER_URL || "http://127.0.0.1:8116";
const OWNER_PRIVATE_KEY = process.env.OWNER_PRIVATE_KEY!;
const XUDT_CODE_HASH = process.env.XUDT_CODE_HASH || "";
const XUDT_HASH_TYPE = (process.env.XUDT_HASH_TYPE || "data1") as "type" | "data" | "data1";
const OWNER_LOCK_HASH = process.env.OWNER_LOCK_HASH || "";

// Configuration — set the address to check, or leave empty to check owner's address
const CHECK_ADDRESS = "";

async function main() {
  lumosConfig.initializeConfig(lumosConfig.predefined.AGGRON4);

  const client = new CkbClient({
    rpcUrl: CKB_RPC_URL,
    indexerUrl: CKB_INDEXER_URL,
  });

  // Determine which address to check
  let address = CHECK_ADDRESS;
  if (!address && OWNER_PRIVATE_KEY) {
    const pubKey = hd.key.privateToPublic(OWNER_PRIVATE_KEY);
    const args = hd.key.publicKeyToBlake160(pubKey);
    const template = lumosConfig.getConfig().SCRIPTS["SECP256K1_BLAKE160"]!;
    address = helpers.encodeToAddress({
      codeHash: template.CODE_HASH,
      hashType: template.HASH_TYPE,
      args: args,
    });
  }

  if (!address) {
    console.error("Error: Set CHECK_ADDRESS in this script or OWNER_PRIVATE_KEY in .env");
    process.exit(1);
  }

  // Compute owner lock hash if not set
  let ownerLockHash = OWNER_LOCK_HASH;
  if (!ownerLockHash && OWNER_PRIVATE_KEY) {
    const pubKey = hd.key.privateToPublic(OWNER_PRIVATE_KEY);
    const args = hd.key.publicKeyToBlake160(pubKey);
    const template = lumosConfig.getConfig().SCRIPTS["SECP256K1_BLAKE160"]!;
    ownerLockHash = computeScriptHash({
      codeHash: template.CODE_HASH,
      hashType: template.HASH_TYPE,
      args: args,
    });
  }

  console.log("=== Checking xUDT Token Balance ===");
  console.log(`Address: ${address}`);
  console.log(`xUDT Code Hash: ${XUDT_CODE_HASH}`);

  try {
    const result = await getTokenBalance(
      client,
      {
        address,
        typeScriptHash: ownerLockHash,
      },
      XUDT_CODE_HASH,
      XUDT_HASH_TYPE,
      ownerLockHash,
    );

    console.log("\n=== Balance Result ===");
    console.log(`Address: ${result.address}`);
    console.log(`Balance: ${result.balance}`);
    console.log(`UDT Cells: ${result.cellCount}`);
  } catch (error: any) {
    console.error("Balance check failed:", error.message);
    process.exit(1);
  }
}

main();
